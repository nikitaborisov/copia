...
const SCORING = {
  ...
  RARITY_REF: 10000,
  STEM_DIRECT: 0.001,    // stem is a literal prefix: rain->rains, sing->singer
  STEM_VARIATION: 0.25,  // spelling-changing stem: bury->buries, mouse->mice
};
...
function createEngine(dictText){
  ...
  /* ---- dictionary ----
     ...
     descending-frequency order. Lines: "word" or "word:base1,base2" where the
     bases are the word's find-list stems from real lemmatization (prefix base
     -> 1/8 penalty, spelling-changing base -> 1/4).
  ...
  const COMMON=[], BONUS=[], RANK=new Map(), STEM_BASES=new Map();
  {
    ...
    for(let i=1;i<lines.length;i++){
      ...
      if(ci>=0) STEM_BASES.set(w,
        line.slice(ci+1).split(',').map(b=>[b, w.startsWith(b)?SCORING.STEM_DIRECT:SCORING.STEM_VARIATION]).sort((a,c)=>a[1]-c[1]));
    }
  }
  ...
  /* ---- scoring ----
     Stem-aware: a word is only discounted if a stem of it is itself on the
     same board's "words to find" list (STEM_DIRECT for suffix add-ons,
     STEM_VARIATION for spelling-changing forms, full weight when the stem is
     absent). STEM_BASES comes from the dictionary artifact (real
     lemmatization at build time). These three functions are the single
     implementation of scoring, used by generation, the UI, and tools. */
